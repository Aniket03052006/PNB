...
async def api_cbom_latest(mode: str = "demo", domain: str | None = None):
    ...
    # If using shallow cache from a base scan, force the unified pipeline to run fully
    if not pipeline_result.get("cbom"):
        pipeline_result = await _ensure_latest_pipeline_result(mode=mode, domain=domain, force_refresh=True)
...
async def api_reports_save(file: UploadFile = File(...)):
    """Save the frontend-generated PDF assessment report to the backend securely."""
    import shutil
    import os
    ...
    if not (safe_filename.endswith(".pdf") or safe_filename.endswith(".json")):
        safe_filename += ".pdf"
    ...
    return JSONResponse(content={"status": "success", "message": "Report saved securely.", "path": str(file_path)})
